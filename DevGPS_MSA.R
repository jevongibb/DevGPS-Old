...
# read input data
Initial_data <- read.table("cbp05msa.txt", header=TRUE, sep=",", stringsAsFactors=FALSE)
# remember to load different Cities/MSAs by different Census periods
Regions <- read.table("Cities_2003.csv", header=TRUE, sep=",", quote="\"", stringsAsFactors=FALSE)
...
# Create RNS for export
RNS <- RNS %>% left_join(Regions, by = c("Region" = "Code"))
write.csv(RNS, "RNS_MSA_2005.csv", row.names = FALSE)
...
percapita_reg_model <- lm(Log_PerCapita ~ RNS + Population + KC0 + Emp, data=RNS2)
...
mygraph <- ggplot(RNS2, aes(x = RNS, y = Log_PerCapita, label = Name)) + geom_point() + geom_smooth(method = 'lm') + labs(x="Economic Network Rating", y="Log GDP Per Capita")
ggplotly(mygraph)
