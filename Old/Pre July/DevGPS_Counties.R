...
Markets <- read.table("Counties.txt", header=TRUE, sep=",", quote="\"", stringsAsFactors=FALSE)
...
Markets$Code <- Markets$fipstate*1000+Markets$fipscty
...
KP0 <- Calc1 %>% group_by(Product) %>% summarise(ProductCount = sum(RCA))
Calc1 <- Calc1 %>% ungroup() # Must ungroup after grouping in the two lines above

Filtered <- spread(Calc1, Product, RCA, fill=0)

# new filter for county-level analysis to remove the small producers that skew data
Filtered$RCA <- KC0$Count # Add RCA column to Template in order to filter
Filtered <- Filtered[Filtered[,2018]>200,] # Remove counties with less than 200 RCA
Filtered$RCA <- NULL # Remove RCA column
Filtered <- Calc3 <- melt(Filtered, id = "Market")
colnames(Filtered) <- c("Market", "Product", "RCA")
Filtered <- Filtered[order(Filtered$Market),]
...
ENI$Market <- formatC(ENI$Market, width = 5, format = "d", flag = "0")
...
GDP$GeoFips <- formatC(GDP$GeoFips, width = 5, format = "d", flag = "0")
ENI <- left_join(ENI, GDP, by=c("Market"="GeoFips"))
