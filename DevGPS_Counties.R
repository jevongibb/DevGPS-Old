...
# create ENI table
RNS <- as.data.frame(Template[, 1])
colnames(RNS) <- "Region"
RNS$RNS <- KCn
RNS$RNS <- c(scale(RNS$RNS))
RNS$Region <- as.integer(RNS$Region)
RNS <- merge(RNS, Regions, by.x = "Region", by.y = "Code")
...
write.csv(ENI, "2014_ENI_CO.csv")
write.csv(PNI, "2014_PNI_CO.csv")
